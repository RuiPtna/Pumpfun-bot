...
function formatConfig(telegramId: number): string {
  ...
  return [
    ...
    `Flux acheteur min : ${p.minBuyRatioPercent}% d'achats — ${p.minNetSolFlow} SOL net — ${p.minUniqueBuyers} acheteurs distincts`,
    ...
    `Pause après pertes : ${p.pauseFeatureEnabled ? "🟢 activée" : "🔴 désactivée"} (${p.consecutiveLossesForPause} pertes consécutives, ${p.pauseDurationMinutes} min)`,
    "",
    "<b>⚡ Exécution</b>",
...
function pnlKeyboard(telegramId: number) {
  ...
  return Markup.inlineKeyboard([
    ...sellButtons,
    [Markup.button.callback("🔄 Actualiser", "menu_pnl")],
    [backToMenuButton()],
  ]);
}
...
function categorizeRejectionReason(reason: string): string {
  const categories: [string, string][] = [
    ...
    ["market cap sorti de la fourchette", "🔪 Couteau qui tombe (juste avant achat)"],
    ["market cap trop faible", "📉 Market cap trop faible"],
    ["market cap trop élevé", "📈 Market cap trop élevé"],
    ["chute récente trop forte", "🔪 Couteau qui tombe (chute 5 min)"],
    ["pas assez de SOL réellement investi", "💧 Pas assez de SOL investi (token mort)"],
    ["progression de la bonding curve", "📊 Progression bonding curve trop faible"],
    ["créateur récidiviste", "🔁 Créateur récidiviste"],
    ["achat initial du créateur trop faible", "👤 Achat initial créateur trop faible"],
    ...
    ["plus de vendeurs que d'acheteurs", "🩸 Pression vendeuse"],
    ["flux SOL net insuffisant", "💧 Flux SOL insuffisant"],
    ["trop peu d'acheteurs distincts", "👤 Trop peu d'acheteurs"],
